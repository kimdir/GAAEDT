...
def print_member(member):
	...
	for attr in attr_list:
		...
		if print_all == True:
			continue
		show_options = True
		while show_options == True:
			show_options = False
			print("\nEnter 'y' to continue listing components.")
			print("Enter 'cls' to clear screen.")
			print("Enter 'all' to print all components.")
			print("Enter 'mat' to print material attributes for current component.")
			print("Enter '' to end component printing.")
			exit_flag = input("\nSelection: ")
			if exit_flag == 'cls':
				...
				mat_properties = getattr(getattr(member,attr),'material_properties')
...
def is_valid_population(pop_list):
	...
	for mem in pop_list:
		if mem.total_fitness == 0:
			invalid_count += 1
	
	if invalid_count >= math.floor(len(pop_list)/3):
		is_valid = False
...
def print_member_fitness(pop_list):
	...
	for mem in pop_list:
		...
		#~ print("Member is defined: " + str(mem.is_defined))
		#~ print("Member is evaluated: " + str(mem.is_evaluated))
		#~ if mem.total_fitness == 0:
			#~ for comp_name,comp in mem.component_dict.items():
				#~ pprint(comp.variable_dict)
			#~ input()
